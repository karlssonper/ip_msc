...
class Wrapper
{
  public:
    Wrapper(const std::string & module) 
    {
        ...
        } else if (module == std::string("GLSL")){
            p = ImageProcessing::OpenGL::Create(false);
        } else if (module == std::string("CUDA")){
            p = ImageProcessing::CCUDA::Create();
        }
    }
    ...
    {
        _AddBuffer(array, true);
    }

    void AddOutputBuffer(const np::ndarray & array)
    {
        _AddBuffer(array, false);
    }
  ...
  private:
    ...
    void _AddBuffer(const np::ndarray & array, bool input)
    {
